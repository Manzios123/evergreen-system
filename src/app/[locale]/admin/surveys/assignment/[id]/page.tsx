...
import SurveyResponseEditor from '@/components/surveys/survey-response-editor';
...
export default function AdminSurveyAssignmentPage({ params }: SurveyAssignmentPageProps) {
  ...
  const { data: myResponse, refetch: refetchMyResponse } = useApiQuery<{ id: string; submitted_at: string; responses: Record<string, any> }>(
    ...
    () => api.get(`/survey-responses/volunteer/assignment/${assignmentId}`),
    { enabled: !!assignmentId && !!isStaffSurveyCompleted }
  );
  ...
  // Staff/admin survey is completed - show their actual answers, with editing
  // if something was wrong or incomplete, instead of a dead-end message.
  if (isStaffSurvey && apiResponse.completed) {
    return (
      <div className="max-w-3xl mx-auto space-y-6">
        <Alert type="success" title="Survey completed">
          <p className="mt-2">
            You completed this survey on {apiResponse.completed_at ? new Date(apiResponse.completed_at).toLocaleDateString() : 'an unknown date'}. You can review or correct your answers below.
          </p>
        </Alert>
        {myResponse ? (
          <SurveyResponseEditor
            ...
            onSaved={() => { refetchMyResponse(); refetch(); }}
          />
        ) : (
          <Card className="p-6">
            <SkeletonLoader type="card" />
